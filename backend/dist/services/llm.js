...
class LLMService {
    async *streamRagResponse(userQuery, contextStr, systemInstruction) {
        dotenv_1.default.config();
        const hfToken = (process.env.HUGGINGFACEHUB_API_TOKEN || '').trim();
        const hfModel = (process.env.HUGGINGFACE_MODEL || 'Qwen/Qwen2.5-7B-Instruct').trim();
        const defaultSystem = 'You are an expert AI assistant answering questions STRICTLY based on the provided PDF document excerpts.\n' +
            'Instructions:\n' +
        ...
        if (hfToken) {
            try {
                const client = new inference_1.HfInference(hfToken);
    ...
    async *streamMockResponse(userQuery) {
        const mockResponse = `Regarding your question ('${userQuery}'): ` +
