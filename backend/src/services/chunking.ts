...
export class ChunkingService {
  ...
  public chunkDocuments(documents: DocumentChunk[]): DocumentChunk[] {
    const nonPassageDocs = documents.filter(
      (doc) => doc.page_content && doc.page_content.trim().length > 0
    );
    if (nonPassageDocs.length === 0) return [];
    ...
    for (const d of splits) {
      ...
      if (total + len > this.chunk_size) {
        ...
        if (currentDoc.length > 0) {
          ...
          if (docStr.trim()) docs.push(docStr);

          // Calculate overlap
          while (
            total > this.chunk_overlap ||
  ...
  public formatChunksSummary(chunkedDocs: DocumentChunk[]): Array<{
    chunk_id: number;
    page: number;
